...
type UserQuery struct {
	...
	withParticipatisRooms *ChatRoomQuery
...
// QueryParticipatisRooms chains the current query on the "participatis_rooms" edge.
func (uq *UserQuery) QueryParticipatisRooms() *ChatRoomQuery {
	query := (&ChatRoomClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) QueryMemberships() *ChatRoomMemberQuery {
	query := (&ChatRoomMemberClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withParticipatisRooms: uq.withParticipatisRooms.Clone(),
...
// WithParticipatisRooms tells the query-builder to eager-load the nodes that are connected to
// the "participatis_rooms" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithParticipatisRooms(opts ...func(*ChatRoomQuery)) *UserQuery {
	query := (&ChatRoomClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withParticipatisRooms = query
	return uq
}
...
func (uq *UserQuery) WithMemberships(opts ...func(*ChatRoomMemberQuery)) *UserQuery {
	query := (&ChatRoomMemberClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withParticipatisRooms; query != nil {
		if err := uq.loadParticipatisRooms(ctx, query, nodes,
			func(n *User) { n.Edges.ParticipatisRooms = []*ChatRoom{} },
			func(n *User, e *ChatRoom) { n.Edges.ParticipatisRooms = append(n.Edges.ParticipatisRooms, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withMemberships; query != nil {
		if err := uq.loadMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.Memberships = []*ChatRoomMember{} },
			func(n *User, e *ChatRoomMember) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadParticipatisRooms(ctx context.Context, query *ChatRoomQuery, nodes []*User, init func(*User), assign func(*User, *ChatRoom)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ParticipatisRoomsTable)
		s.Join(joinT).On(s.C(chatroom.FieldID), joinT.C(user.ParticipatisRoomsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.ParticipatisRoomsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ParticipatisRoomsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*ChatRoom](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "participatis_rooms" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadMemberships(ctx context.Context, query *ChatRoomMemberQuery, nodes []*User, init func(*User), assign func(*User, *ChatRoomMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chatroommember.FieldUserID)
	}
