...
// The ChatRoomMemberFunc type is an adapter to allow the use of ordinary
// function as ChatRoomMember mutator.
type ChatRoomMemberFunc func(context.Context, *ent.ChatRoomMemberMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChatRoomMemberFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ChatRoomMemberMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChatRoomMemberMutation", m)
}
